...
// dependency-update-policy.mjs and have green CI.
//
...
// privileged path is covered by tests/unit/dependency-auto-merge.test.ts; the
  ...
  return async function api(path, options = {}) {
    ...
    if (!response.ok) {
      const error = new Error(
        `${options.method || "GET"} ${path} -> ${response.status}: ${body?.message || text}`,
      );
...
function whole(items, what) {
  if (items.length >= PAGE_SIZE) {
    throw new Error(
      `more than ${PAGE_SIZE} ${what}; refusing to judge a partial list`,
    );
  }
  return items;
}
 ...
 */
export function verifyProvenance(commits, author) {
  const misattributed = commits.filter(
    (commit) => commit.author?.login !== author,
  );
  if (misattributed.length) {
    return `${misattributed.length} commit(s) not authored by \`${author}\``;
  }
  const pushed = commits.filter(
    (commit) => !ACCEPTED_COMMITTERS.includes(commit.committer?.login),
  );
  if (pushed.length) {
    const who = pushed.map((commit) => commit.committer?.login ?? "nobody");
  ...
  if (unsigned.length) {
    return (
      `${unsigned.length} commit(s) carry no verified signature, so the author ` +
 ...
 */
export function pullRequestStateProblem(pr, { repo, baseBranches }) {
  if (pr.state !== "open") return "not open";
  if (pr.draft) return "draft";
  if (pr.head.repo?.full_name !== repo) return "head is a fork";
  if (!baseBranches.includes(pr.base.ref)) {
...
async function readJson(path, ref, { api, repo }) {
  try {
    return JSON.parse(
      await api(`/repos/${repo}/contents/${path}?ref=${ref}`, { accept: RAW }),
    );
  } catch (error) {
...
async function classifyFileContents(changedFiles, pr, { api, repo }) {
  for (const path of changedFiles.filter((file) => MANIFEST.test(file))) {
    ...
    if (!verdict.safe)
      return { safe: false, reason: `${path} ${verdict.reason}` };
  }
 ...
 */
export async function evaluatePullRequest(number, { api, repo, config }) {
  const {
    baseBranches = [],
    requiredChecks = [],
    allowedUpdates = {},
  ...
  if (comparison.total_commits > commits.length) {
    throw new Error(
      `only ${commits.length} of ${comparison.total_commits} commits were listed; ` +
        "refusing to judge a partial list",
    );
  }
  if (comparison.files.length >= MAX_FILES) {
    throw new Error(
      `${comparison.files.length} changed files; refusing to judge a partial list`,
    );
  }
  ...
  const combinedStatus = await api(
  ...
  if (current.head.sha !== pr.head.sha) {
    return skip(
      `head moved from \`${pr.head.sha.slice(0, 8)}\` to \`${current.head.sha.slice(0, 8)}\` during inspection`,
    );
  }
  ...
  const objector = [...latestByReviewer].find(
...
export async function run({ api, repo, config, prNumbers, log = console }) {
  const results = [];
...
async function main() {
  ...
  if (prNumbers.length === 0) {
    console.log("No pull requests to evaluate.");
  }
  ...
  if (process.env.GITHUB_STEP_SUMMARY && results.length) {
    const lines = results.map(
      (result) =>
        `- **#${result.number}**: ${result.outcome} -- ${result.reason}`,
    );
    appendFileSync(
