...
async function evaluate(number) {
  ...
  if (pr.head.repo?.full_name !== repo) return skip(number, "head is a fork");
  if (!baseBranches.includes(pr.base.ref)) {
    return skip(
      number,
      `targets \`${pr.base.ref}\`, not ${baseBranches.join(" or ")}`,
    );
  }
  ...
  if (blocking.length) {
    return skip(number, `carries the \`${blocking[0]}\` label`);
  }
