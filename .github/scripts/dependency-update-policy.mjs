...
export function parseRemovals(message) {
  const removals = new Set();
  for (const [, name] of message.matchAll(/^Removes `([^`]+)`/gm)) {
    removals.add(name);
  }
  return removals;
}
...
export function classifyDependabot(messages, changedFiles, allowedUpdates) {
  ...
  for (const message of messages) {
    ...
    for (const name of parseRemovals(message)) removals.add(name);
  }
  if (entries.length === 0) {
    return {
      safe: false,
  ...
  for (const entry of entries) {
    const dependencyTypeOf = entry.dependencyType || "unknown";
    if (removals.has(entry.name)) {
      ...
      if (dependencyTypeOf !== "indirect") {
        return {
          safe: false,
          reason: `\`${entry.name}\` is being removed (${dependencyTypeOf}; only indirect removals qualify)`,
        };
      }
    ...
    if (!allowed.includes(bump)) {
      return {
        safe: false,
        reason: `\`${entry.name}\` is a ${bump} bump (${dependencyType}; allowed: ${allowed.join(", ")})`,
      };
    }
    described.push(
...
export function classifyManifestChange(before, after) {
  ...
  const changedOutside = [...keys].filter(
    (key) =>
      !DEPENDENCY_KEYS.includes(key) &&
      JSON.stringify(before[key]) !== JSON.stringify(after[key]),
  );
  if (changedOutside.length) {
    return {
      safe: false,
      reason: `changes \`${changedOutside.join("`, `")}\` in package.json, which is not a dependency map`,
    };
  }
 ...
 */
export function classifyLockfile(lockfile) {
  const entries = Object.entries(lockfile.packages || {});
  const offRegistry = entries.filter(
    ([, entry]) => entry?.resolved && !entry.resolved.startsWith(REGISTRY),
  );
  if (offRegistry.length) {
    const [name, entry] = offRegistry[0];
    return {
      safe: false,
      reason: `resolves \`${name || "the root package"}\` from \`${entry.resolved}\`, not the npm registry`,
    };
  }
  return { safe: true };
}
