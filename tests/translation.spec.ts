...
test.describe('Google Translate simulation', () => {
  test('home page renders with simulated translate wrappers', async ({ page }) => {
    await simulateGoogleTranslate(page);
    await page.goto('/');

    await expect(page.locator('h1')).toContainText('Free support services validated by your peers');
